...
// ---------- 签名调试：不发请求，只展示我们本地生成签名的每一步 ----------
// 部署后打开 /api/debug/sign，把这里的 reqTime / 拼接字符串 / authcode
// 和 OMS 后台【API信息 - 开发验签工具】里输入同样的 data 后生成的结果对比，
// 一旦某一步不一致，就能精确定位到底是签名算法哪里错了。
app.get('/api/debug/sign', (req, res) => {
  if (DEMO_MODE) {
    return res.status(400).json({ success: false, message: '演示模式下没有 appKey/appSecret，无法生成签名' });
  }
  const { debugSign } = require('./sign');
  const reqTime = String(Math.floor(Date.now() / 1000));
  const data = { page: 1, pageSize: 10 }; // 和 /api/products 用的参数一致，方便直接对比
  const originalLog = console.log;
  const lines = [];
  console.log = (...args) => lines.push(args.join(' '));
  const authcode = debugSign({ appKey: XLWMS_APP_KEY, data, reqTime }, XLWMS_APP_SECRET);
  ...
  res.json({
    ...
    data: {
      ...
      authcode,
      steps: lines,
      tip: '把上面的 data / reqTime / authcode 拿到 OMS 后台【API信息-开发验签工具】里核对，看生成的 authcode 是否一致',
    },
  });
});
